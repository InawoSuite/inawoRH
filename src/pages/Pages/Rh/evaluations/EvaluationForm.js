import React, {useState, useMemo, useMemouseState, useEffect, useRef } from "react";
import {
  Container,
  Row,
  Col,
...
const EvaluationForm = ({ mode = "add" }) => {
  ...
  useEffect(() => {
    formData.quizzes.forEach((quiz, quizIndex) => {
      quiz.questions.forEach((question, questionIndex) => {
        ...
        if (
          explicationQuillRefs.current[refKey] &&
          !explicationQuills[refKey]
        ) {
          ...
          setExplicationQuills((prev) => ({
            ...prev,
            [refKey]: quillInstance,
          }));
  ...
  useEffect(() => {
    if (mode === "edit" && id) {
      ...
      setSelectedModeEvaluation(
        modeEvaluationOptions.find(
          (opt) => opt.value === mockData.modeEvaluation,
        ),
      );
    }
  }, [mode, id]);
  ...
  const removeQuestion = (quizIndex, questionIndex) => {
    const updated = [...formData.quizzes];
    updated[quizIndex].questions = updated[quizIndex].questions.filter(
  ...
  const removeOption = (quizIndex, questionIndex, optionIndex) => {
    const updated = [...formData.quizzes];
    updated[quizIndex].questions[questionIndex].options = updated[
      quizIndex
  ...
  const handleOptionChange = (quizIndex, questionIndex, optionIndex, value) => {
    const updated = [...formData.quizzes];
    updated[quizIndex].questions[questionIndex].options[optionIndex].texte =
      value;
    setFormData((prev) => ({
      ...prev,
      quizzes: updated,
                        ...
                        <CardBody>
                          ...
                          <Row>
                            ...
                            <Col md={4}>
                              <FormGroup className="mb-3">
                                <Label>{t("Type")}</Label>
                                <CustomSelect
                                  value={typeQuestionOptions.find(
                                    (opt) => opt.value === quiz.type,
                                  )}
                                  onChange={(opt) =>
                                    handleQuizChange(
                                      quizIndex,
                          ...
                          <FormGroup className="mb-3">
                            ...
                            >
                              <div
                                ref={(el) =>
                                  (quizQuillRefs.current[`quiz-${quizIndex}`] =
                                    el)
                                }
                          ...
                          <div className="mt-4">
                            ...
                            {quiz.questions.map((question, questionIndex) => (
                              <Card
                                key={questionIndex}
                                className="mb-3 border-0 bg-light rounded-4"
                              >
                                <CardBody>
                                  <div className="d-flex justify-content-between align-items-center mb-3">
                                    <h6 className="mb-0">
                                  ...
                                  <FormGroup className="mb-3">
                                    ...
                                    >
                                      <div
                                        ref={(el) =>
                                          (questionQuillRefs.current[
                                            `question-${quizIndex}-${questionIndex}`
                                          ] = el)
                                        }
                                        style={{ minHeight: "100px" }}
                                      />
                                    ...
                                    <div className="d-flex align-items-center gap-3">
                                      <div
                                        ...
                                        onClick={() =>
                                          document
                                            .getElementById(
                                              `image-upload-${quizIndex}-${questionIndex}`,
                                            )
                                            .click()
                                        }
                                      >
                                  ...
                                  <FormGroup className="mt-3">
                                    ...
                                    >
                                      <div
                                        ref={(el) =>
                                          (explicationQuillRefs.current[
                                            `explication-${quizIndex}-${questionIndex}`
                                          ] = el)
                                        }
                                        style={{ minHeight: "100px" }}
                                      />
