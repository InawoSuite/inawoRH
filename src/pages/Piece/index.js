...
const Piece = () => {
  ...
  const fetchPieces = useCallback(async () => {
    ...
    setTimeout(() => {
      try {
        ...
        toast.success("Données des pièces comptables chargées avec succès !");
      } catch (err) {
  ...
  // ✅ Chargement initial
  useEffect(() => {
    document.title = "Pièces Comptables | INAWO - Suite de Gestion";
    fetchPieces();
  }, [fetchPieces]);
      ...
      <div className="page-content">
        <Container fluid>
          ...
          <DeleteModal
            ...
            deleteMessage="Êtes-vous sûr de vouloir supprimer cette pièce comptable ? Cette action est irréversible."
          />
              ...
              {loading ? (
                <div className="d-flex justify-content-center align-items-center my-5" style={{ minHeight: '300px' }}>
                  <div className="text-center">
                    <Loader />
                    <p className="mt-3 text-muted">Chargement des pièces comptables...</p>
                  </div>
                </div>
                ...
                <EmptyDataCard
                  title="Aucune pièce comptable trouvée"
                  description={
                    searchTerm
                      ? `Aucun résultat pour "${searchTerm}". Essayez avec d'autres termes.`
                      : "Commencez par créer ou importer des pièces comptables."
                    }
                  actionButton={
                    <Button
                      color="success"
